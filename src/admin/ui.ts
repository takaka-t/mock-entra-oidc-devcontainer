...
<label>Allowed scopes</label><div class="checks"><label><input type="checkbox" name="clientScope" value="openid" checked disabled>openid</label><label><input type="checkbox" name="clientScope" value="profile">profile</label><label><input type="checkbox" name="clientScope" value="email">email</label><label><input type="checkbox" name="clientScope" value="offline_access">offline_access</label></div>
...
$('clientForm').onsubmit=async event=>{event.preventDefault();const type=$('clientType').value,payload={clientType:type,...(type==='CONFIDENTIAL'?{clientSecret:$('clientSecret').value,tokenEndpointAuthMethod:$('authMethod').value}:{tokenEndpointAuthMethod:'none'}),redirectUris:lines($('redirectUris').value),postLogoutRedirectUris:lines($('logoutUris').value),scopes:[...document.querySelectorAll('[name=clientScope]:checked')].map(x=>x.value),accessTokenAudience:$('audience').value};try{const url=editingClientId?'/__mock/api/clients/'+encodeURIComponent(editingClientId):'/__mock/api/clients';await request(url,{method:editingClientId?'PUT':'POST',headers:{'content-type':'application/json'},body:JSON.stringify(editingClientId?payload:{clientId:$('clientId').value,...payload})});$('clientEditor').classList.add('hidden');await loadClients()}catch(error){$('editorError').textContent=message(error)}};
$('resetClients').onclick=async()=>{if(!confirm('Reset all OIDC clients to defaults?'))return;try{renderClients(await request('/__mock/api/clients/reset',{method:'POST'}));$('clientError').textContent=''}catch(error){$('clientError').textContent=message(error)}};clientTypeFields();void loadClients();
</script></body></html>`;
