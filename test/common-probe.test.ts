...
describe.each(["", "/tenant/v2.0"])(
  "common probe with issuer path '%s'",
  (issuerPath) => {
    ...
    it.each([
      "/%63ommon/oauth2/v2.0/authorize",
      "/common/oauth2/v2.0/%61uthorize/",
      "/common/oauth2/v2%2E0/authorize",
    ])("enforces origin, CORS, and fault counts for %s", async (url) => {
      for (const scenario of [
        ...
        expect(response.statusCode).toBe(
          scenario === "AUTH_429" ? 429 : scenario === "AUTH_500" ? 500 : 200,
        );
    ...
    it.each([
      ...
      "does not reinterpret a different route or consume a fault: %s",
      async (url) => {
        context.store.set({
          scenario: "AUTH_500",
          mode: "LIMITED",
